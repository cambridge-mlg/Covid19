...
function country_computation!(
    expected_daily_cases_m, expected_daily_deaths_m, cases_pred_m, Rₜ_m, Rₜ_adj_m,
    ...
    Rₜ_m .= μ_m * exp.(xs * (-α) + (- lockdown_m) * xs[:, lockdown_index])
...
@threads for m = 1:num_countries
    country_computation!(
        expected_daily_cases[m], expected_daily_deaths[m], cases_pred[m], Rₜ[m], Rₜ_adj[m], # <= mutated
