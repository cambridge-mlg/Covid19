...
\begin{align}
  ...
  R_{t, m} &= \mu_m \exp\bigg(- \beta_m x_{k_{\text{ld}}} - \sum_{k=1}^{K} \alpha_k x_k\bigg) \quad & \text{for} \quad m = 1, \dots, M, \ t = 1, \dots, T  \\
  \tilde{R}_{t, m} &= \bigg( 1 - \frac{1}{p_m} \sum_{\tau = 1}^{t - 1} c_{\tau, m} \bigg) R_{t, m}  \quad & \text{for} \quad m = 1, \dots, M, \ t = T_{\text{impute}} + 1, \dots, T \\
